        ...
        <div class="left" >
            ...
            <div class="pictureBox" >
                <div class="swiper-box" >
                    <div v-if="pics.length>0 && pics[0].pictures" :class="['swiper-gallery', {hover: hover}]" @mouseenter.stop.prevent="mouseenter" @mouseleave.stop.prevent="mouseleave">
                        <swiper v-if="showSwiper && pics[0].pictures.length>0" class="swiper gallery-top swiper-no-swiping" :options="swiperOptionTop" ref="swiperTop">
                            <swiper-slide v-for="(e,i) in pics[0].pictures" :key="i" :data-swiper-autoplay="value">
                                <div class="img-box" @mousewheel.prevent="rollImg"><img class="swiper-lazy" style="transform:scale(1)" :data-src="reDomain(e.imageDir, e.imageName)" :ref="img_index ==i?'imgDiv':''" alt="" @mousedown="move"></div>
                                <div class="swiper-lazy-preloader"></div>
                            </swiper-slide>
                        </swiper>
                        <template v-if="pics.length>0 && pics[0].pictures && pics[0].pictures.length>0">
                    ...
                    <div class="top_hover">
                        ...
                        <div class="tools">
                            <template v-if="pics.length>0 && pics[0].pictures && pics[0].pictures.length>0">
                                ...
                                <div class="item">
                                    <el-button size="mini" type="danger" @click="alarmClick(pics[0].pictures[img_index],pics[0].yuntai)">手动告警</el-button>
                                </div>
                                <div class="item">
                                    <span>翻页间隔(秒)</span>
                    ...
                    <div class="swiper-thumbnail"  @click.stop="noautoplay">
                        <div class="swiper gallery-thumbs">
                            <template  v-if="pics.length>0 && pics[0].pictures && pics[0].pictures.length>0" >
                                <div :class="['swiper-slide', {act:img_index == i}]" v-for="(e,i) in pics[0].pictures" :key="i" @click="slideto(i)">                        
                                    <div class="box">
                                        ...
                                        <img :src="reDomain(e.imageDir, e.imageName)">
                                    </div>
                                </div>
                            </template>
                        </div>
            ...
            <div class="more" @click="hideclick">{{open?'更多功能':'隐藏功能'}}</div>
        </div>
        <div class="right">
...
export default {
    ...
    data () {
        ...
        return {
            ...
            pics: [],
            thisPicInfo: {captureTime:undefined,captureUser:undefined,presetBit:undefined,shape:undefined,day:undefined},
            value: 5000,
            yuntaiinfo: {},// 云台信息
            queryParams: {pageNum:1,pageSize:8,yuntaiId:undefined,day:undefined,presetBit:undefined},
            ...
            swiperOptionTop: {
                ...
                on:{
                    slideChange: function(){
                        ...
                        self.preset_bit = self.pics[0].pictures[this.activeIndex].presetBit;
                        self.resetMeths(); // 重置拖拽，缩小，放大
                        self.thisPicInfo['captureTime'] = self.pics[0].pictures[this.activeIndex].captureTime; // 替换当前图片标题
                        self.thisPicInfo['captureUser'] = self.pics[0].pictures[this.activeIndex].captureUser; // 替换当前图片标题
                        self.thisPicInfo['presetBit'] = self.pics[0].pictures[this.activeIndex].presetBit; // 替换当前图片标题
                        self.thisPicInfo['shape'] = self.pics[0].pictures[this.activeIndex].shape;
                        // 当前页面最后一张时触发
                        if(self.autoplay) {
                            // console.log("当前页面最后一张时触发");
                            // console.log("self.value",self.value);
                            // console.log("self.pics[0].pictures.length",self.pics[0].pictures.length);
                            // console.log("self.queryParams.pageNum",self.queryParams.pageNum);
                            // console.log("this.activeIndex",this.activeIndex);
                            if(this.activeIndex == (parseInt(self.pics[0].pictures.length) - 1) ) {
                                console.log("1");
    ...
    mounted() {   
        ...
        this.queryParams.day = this.imgcheck && this.imgcheck.day;
        console.log(">>>>>>>this.queryParams.day",this.imgcheck && this.imgcheck.day);
    ...
    methods: {
        filterdata (data) {
            ...
            this.thisPicInfo['captureTime'] = this.pics[0] && this.pics[0].pictures && this.pics[0].pictures.length>0 && this.pics[0].pictures[0].captureTime;
            this.thisPicInfo['captureUser'] = this.pics[0] && this.pics[0].pictures && this.pics[0].pictures.length>0 && this.pics[0].pictures[0].captureUser;
            this.thisPicInfo['shape'] = this.pics[0] && this.pics[0].pictures && this.pics[0].pictures.length>0 && this.pics[0].pictures[0].shape;
            this.thisPicInfo['presetBit'] = this.pics[0] && this.pics[0].pictures && this.pics[0].pictures.length>0?this.pics[0].pictures[0].presetBit:this.queryParams.presetBit;
        ...
        autoplay_click (){          
            ...
            if(this.queryParams.pageNum == this.pageTotal() && (parseInt(this.img_index)+1) == this.pics[0].pictures.length) {
        ...
        slideto (idx) {
            ...
            this.preset_bit = this.pics[0].pictures[idx].presetBit;
        },
        less_preset_init() {
            // 上一个预置位
            ...
            get_pictureListByDetails(self.queryParams).then(response => {
                if(response.code == 200 && response.rows && response.rows.length>0) {
                    ...
                    if(self.pics && self.pics.length>0) {                        
                        self.thisPicInfo['shape'] = self.pics[0] && self.pics[0].pictures && self.pics[0].pictures.length>0 && self.pics[0].pictures[0].shape;
                        self.thisPicInfo['captureTime'] = self.pics[0] && self.pics[0].pictures && self.pics[0].pictures.length>0 && self.pics[0].pictures[0].captureTime;
                        self.thisPicInfo['captureUser'] = self.pics[0] && self.pics[0].pictures && self.pics[0].pictures.length>0 && self.pics[0].pictures[0].captureUser;
                        self.thisPicInfo['presetBit'] = self.pics[0] && self.pics[0].pictures && self.pics[0].pictures.length>0? self.pics[0].pictures[0].presetBit:self.thisPicInfo.presetBit;
                        if(self.autoplay) {
                            this.$nextTick(()=>{
                                console.log(111)
        ...
        arrowright() {
            if((parseInt(this.img_index) + 1) == this.pics[0].pictures.length && this.queryParams.pageNum < this.pageTotal()) {
                this.queryParams.pageNum++;
                this.getPics();
            }
            this.noautoplay();
