<template>
    <div class="pictureBox">
        <div class="swiper-box">
            ...
            <div class="top_hover">
                <div class="info">
                    <span class="title">{{yuntaiInfo.lineName}}{{yuntaiInfo.towerName}}{{yuntaiInfo.yuntaiName}}</span><span><font>{{Object.keys(thisPicInfo).length>0&&thisPicInfo.shape&&thisPicInfo.shape.length>0?dicts.tower_shape.obj[thisPicInfo.shape].dictLabel:''}}</font></span><span>预置位：{{Object.keys(thisPicInfo).length>0?thisPicInfo.presetBit:''}}</span><span>{{Object.keys(thisPicInfo).length>0?thisPicInfo.captureTime:''}}</span>
                </div>
                <div class="tools">
                    <template v-if="pics.length>0">
                        <!-- 下载 -->
                        ...
                        <!-- <div class="item">
                            <el-button size="mini" type="danger" @click="handClick">手动告警</el-button>
                        </div> -->
                        <div class="item">
                            <span>翻页间隔(秒)</span>
                    ...
                    <div class="item">
                        <el-date-picker @change="changeDate" size="mini" style="transform: scale(0.8);transform-origin: right;width:200px;" :picker-options="expireTimeOption" format="yyyy-MM-dd" clear-icon="" value-format="yyyy-MM-dd" v-model="queryParams.day" type="date" placeholder="选择日期"></el-date-picker>
                        ...
                        <div :class="['swiper-slide', {act:img_index == i}]" v-for="(e,i) in pics" :key="i" @click="slideto(i)">                        
                            <div class="box">
                                ...
                                <div class="mask">
                                    ...
                                    <span>{{e.captureTime?e.captureTime:'暂无时间'}}</span>
                                </div>
...
export default {
    ...
    mounted() {   
        ...
        this.queryParams.presetBit = this.presetBit;
    ...
    methods: {
        handClick () {
            ...
            // 手动告警      
            if(this.thisPicInfo.isAlarm == 1) {this.msgSuccess("该图片已告警"); return;}
            this.$parent.$parent.alarmClick(this.thisPicInfo,this.yuntaiInfo);
        ...
        downloadImg() {
            // 下载当前图片
            let imgsrc = this.reDomain(this.thisPicInfo.imageDir, this.thisPicInfo.imageName);
        ...
        less_preset_init() {
            // 上一个预置位
            // console.log("点击上一个预置位");
            this.noautoplay();
            // console.log("当前预置位",this.thisPicInfo.presetBit);
            let preset_bit = parseInt(this.queryParams.presetBit), PRESET_BIT_TOTAL = parseInt(global.PRESET_BIT_TOTAL);
            // if(!this.thisPicInfo.presetBit){return;}
            if(preset_bit==0) {
                // console.log("当预置位等于0时",preset_bit);  
                preset_bit = PRESET_BIT_TOTAL;
                // return;
            } else {
                preset_bit = preset_bit - 1;
                // console.log("上一个预置位",preset_bit);
            }       
        ...
        add_preset_init() {
            // 下一个预置位
            // console.log("点击下一个预置位");
            this.noautoplay();
            // console.log("当前预置位",this.thisPicInfo.presetBit);
            let preset_bit = parseInt(this.queryParams.presetBit), PRESET_BIT_TOTAL = parseInt(global.PRESET_BIT_TOTAL);
            // if(!this.thisPicInfo.presetBit){return;}
            if(preset_bit == PRESET_BIT_TOTAL) {    
                // console.log("当预置位超过255时",preset_bit);           
                preset_bit = 0;
            } else {
                preset_bit = preset_bit + 1;
                // console.log("下一个预置位",preset_bit);
            }
            this.preset_bit = preset_bit;
            this.queryParams.presetBit = this.preset_bit;
            this.queryParams.pageNum =  1;
        ...
        getPictureList () {
            ...
            console.log("preset_bit :",this.queryParams.presetBit);
                if(response.code == 200 && response.rows.length>0) {
                    if(response.rows[0].pictures.length>0) {
                        self.pics = response.rows[0].pictures;
                        self.thisPicInfo = response.rows[0].pictures[0];
                    }
        ...
        close () {       
           ...
           this.$parent.$parent.get_report_pictureList();
        },
        update () {
            let self = this;
