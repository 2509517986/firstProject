<template>
    <el-card class="app-container">        
                    ...
                    <div class="tools">
                        <el-form :inline="true" :model="queryParams" ref="formInline" size="mini">
                            <el-form-item label="预置位" prop="presetBit">
                                <el-select v-model="queryParams.presetBit" filterable  placeholder="请选择" size="mini" clearable>
                                    <el-option v-for="(item,index) in bits" :key="index" :label="item" :value="item"></el-option>
                                </el-select>
                            </el-form-item>
                            <el-form-item label="" prop="day">                        
                                <el-date-picker size="mini" :picker-options="expireTimeOption" format="yyyy-MM-dd" clear-icon="" value-format="yyyy-MM-dd" v-model="queryParams.day" type="date" placeholder="选择日期"></el-date-picker>
                            </el-form-item> 
                            <el-form-item>
                    ...
                    <div class="listcon">
                        <el-card shadow="never" class="item" v-for="(e,i) in pics" :key="i" :body-style="{paddingLeft: '0px',paddingRight: '0px',paddingTop:'20px',paddingBottom:'0'}">
                            <div slot="header" class="elcard-top">
                                ...
                                <el-tag size="small">{{Object.keys(dicts).length>0 && dicts.oem.obj[e.yuntai.factory]&& dicts.oem.obj[e.yuntai.factory].dictLabel}}</el-tag>
                                <div class="btns">
                        ...
                        <div class="item" v-for="(e,i) in pics" :key="i">
                            <div class="itemcon">
                                ...
                                <div class="tools" v-if="e.pictures && e.pictures.length>0">
                                    <span>{{Object.keys(dicts).length>0 && dicts.oem.obj[e.yuntai.factory] && dicts.oem.obj[e.yuntai.factory].dictLabel}}</span>
                                    <span>{{Object.keys(dicts).length>0 && dicts.monitor_type.obj[e.yuntai.monitorType] && dicts.monitor_type.obj[e.yuntai.monitorType].dictLabel}}</span>
        ...
        <!-- 显示详情 -->
        <div :class="['showdetailbox',{left: show == 2}]" v-if="show == 1 || show == 2" ><picture-info @close="close" :imgcheck="imgcheckdata"></picture-info></div>
        <!-- 手动抓拍图片 -->
        ...
        <!-- 手动告警 -->
        <manual-alarm v-if="visible3" :alarminfo="thisImgInfo" :yuntaiinfo="thisyuntai"></manual-alarm>
    </el-card>
</template>
<script>
...
export default {
    data () {
        return {
            ...
            thisyuntai: {}, // 当前云台信息
            total: 150,
            queryParams: {pageNum:1,pageSize:4,lineId:undefined,towerId:undefined,yuntaiIds:undefined,day: undefined,presetBit:undefined,imgSize:4},
            layout: true,
            pics: [],
            getTreedata:{}, // 树结构数据
            imgcheckdata: {presetBit:1,day:undefined,yuntaiId:undefined,single:false,state:undefined}, // 传递到图片详情
            expireTimeOption: {
                // 当天后不可以选择
                disabledDate(date) {
    ...
    methods: {
        ...
        reset() {
            ...
            this.imgcheckdata.state = undefined;
            this.imgcheckdata.day = this.queryParams.day;
        ...
        ccc(data) {
            ...
            else if(data.node[0].node.type == "tower") {this.show = 0;this.queryParams.towerId = data.node[0].node.id;this.queryParams.lineId = data.node[0].node.parentId;}
            else if(data.node[0].node.type == "yuntai") {this.$nextTick(()=>{this.show = 2;this.queryParams.yuntaiIds = data.node[0].node.id;this.imgcheckdata.presetBit= this.queryParams.presetBit;this.imgcheckdata.yuntaiId = this.queryParams.yuntaiIds;});this.thisyuntai= data.node[0].node;this.imgcheckdata.state = data.node[0].node.state;this.queryParams.towerId = data.node[0].node.parentId,this.queryParams.lineId=data.node[0].node.lineId;this.imgcheckdata.day = this.queryParams.day;}
            if(data.node[0].node.type != "yuntai") {
        ...
        /** 查询列表 */
        getPictureList() {
            get_pictureListByDay(this.queryParams).then(response => {
        ...
        .listcon{flex:1;overflow-y:auto;overflow-x:hidden;
            .item{border-radius: 0;border:0;padding:0;padding-top:$base-padding;&:first-child{padding-top:0;}
                ...
                .elcard-top{display:flex;align-items:center;
                    .name{font-size:13px;}
                    .yuntaiIcon{margin-right:3px;font-size:14px;color:$base-menu-color;&.online{color:$base-green-color;}}
