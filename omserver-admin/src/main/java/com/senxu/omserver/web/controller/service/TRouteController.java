package com.senxu.omserver.web.controller.service;

import java.util.List;

import com.senxu.omserver.service.domain.TRoutePoint;
import com.senxu.omserver.service.service.ITRoutePointService;
import org.springframework.security.access.prepost.PreAuthorize;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;
import com.senxu.omserver.common.annotation.Log;
import com.senxu.omserver.common.core.controller.BaseController;
import com.senxu.omserver.common.core.domain.AjaxResult;
import com.senxu.omserver.common.enums.BusinessType;
import com.senxu.omserver.service.domain.TRoute;
import com.senxu.omserver.service.service.ITRouteService;
import com.senxu.omserver.common.utils.poi.ExcelUtil;
import com.senxu.omserver.common.core.page.TableDataInfo;

/**
 * t_route（航线信息）Controller
 * 
 * @author ruoyi
 * @date 2022-01-25
 */
@RestController
@RequestMapping("/system/route")
public class TRouteController extends BaseController
{
    @Autowired
    private ITRouteService tRouteService;
    @Autowired
    private ITRoutePointService tRoutePointService;

    /**
     * 查询t_route（航线信息）列表
     */
    //@PreAuthorize("@ss.hasPermi('system:route:list')")
    @GetMapping("/list")
    public TableDataInfo list(TRoute tRoute)
    {
        startPage();
        List<TRoute> list = tRouteService.selectTRouteList(tRoute);
        for (TRoute route : list){
            List<TRoutePoint> routePoints = tRoutePointService.selectTRoutePointList(new TRoutePoint(route.getRouteId()));
            route.settRoutePoint(routePoints);
        }
        return getDataTable(list);
    }

    /**
     * 导出t_route（航线信息）列表
     */
    //@PreAuthorize("@ss.hasPermi('system:route:export')")
    @Log(title = "t_route（航线信息）", businessType = BusinessType.EXPORT)
    @GetMapping("/export")
    public AjaxResult export(TRoute tRoute)
    {
        List<TRoute> list = tRouteService.selectTRouteList(tRoute);
        ExcelUtil<TRoute> util = new ExcelUtil<TRoute>(TRoute.class);
        return util.exportExcel(list, "t_route（航线信息）数据");
    }

    /**
     * 获取t_route（航线信息）详细信息
     */
    //@PreAuthorize("@ss.hasPermi('system:route:query')")
    @GetMapping(value = "/{routeId}")
    public AjaxResult getInfo(@PathVariable("routeId") Long routeId)
    {
        return AjaxResult.success(tRouteService.selectTRouteById(routeId));
    }

    /**
     * 获取t_route（航线信息）详细信息
     */
    //@PreAuthorize("@ss.hasPermi('system:route:query')")
    @GetMapping(value = "/task/{taskId}")
    public AjaxResult getInfoByTask(@PathVariable("taskId") Long taskId)
    {
        System.out.println(tRouteService.selectTRouteByTaskId(taskId).toString());
        return AjaxResult.success(tRouteService.selectTRouteByTaskId(taskId));
    }

    /**
     * 新增t_route（航线信息）
     */
    //@PreAuthorize("@ss.hasPermi('system:route:add')")
    @Log(title = "t_route（航线信息）", businessType = BusinessType.INSERT)
    @PostMapping
    public AjaxResult add(@RequestBody TRoute tRoute)
    {
        return toAjax(tRouteService.insertTRoute(tRoute));
    }

    /**
     * 修改t_route（航线信息）
     */
    //@PreAuthorize("@ss.hasPermi('system:route:edit')")
    @Log(title = "t_route（航线信息）", businessType = BusinessType.UPDATE)
    @PutMapping
    public AjaxResult edit(@RequestBody TRoute tRoute)
    {
        return toAjax(tRouteService.updateTRoute(tRoute));
    }

    /**
     * 删除t_route（航线信息）
     */
    //@PreAuthorize("@ss.hasPermi('system:route:remove')")
    @Log(title = "t_route（航线信息）", businessType = BusinessType.DELETE)
	@DeleteMapping("/{routeIds}")
    public AjaxResult remove(@PathVariable Long[] routeIds)
    {
        return toAjax(tRouteService.deleteTRouteByIds(routeIds));
    }
}
